...
export const NODE_JOB_TAGS = ["Business", "UX", "Tech"];
export const NODE_STT_TOPIC_TAGS = ["Context", "Memory", "STT"];
export const STT_MEETING_PROJECT_ID = "project-ai-meeting";

export const TOPIC_TAG_META = {
  Context: { bg: "#EFEBFF" },
  Memory: { bg: "#D4CDF1" },
  STT: { bg: "#CAC3E4" },
  Research: { bg: "#EFEBFF" },
  Validation: { bg: "#D4CDF1" },
  Insight: { bg: "#CAC3E4" },
  Reference: { bg: "#EFEBFF" },
  Decision: { bg: "#D4CDF1" },
  Action: { bg: "#CAC3E4" },
...
export const JOB_TAG_META = {
  Business: { bg: "#E0F8DB", className: "bg-[#E0F8DB] text-[#4F4F4F]" },
  UX: { bg: "#FFD7FE", className: "bg-[#FFD7FE] text-[#4F4F4F]" },
  Tech: { bg: "#94CDFF", className: "bg-[#94CDFF] text-[#4F4F4F]" },
};
...
function inferJobTag({ category, phase, title, content } = {}) {
  const text = `${title || ""} ${content || ""}`.toLowerCase();
  if (/\b(api|backend|server|database|infra|architecture|deploy|code|dev|engineer|llm|stt|model|tech|개발|구현|서버|인프라|기술|아키텍처)\b/.test(text)) {
    return "Tech";
  }
  if (/\b(ux|ui|design|wireframe|screen|flow|interaction|user experience|디자인|화면|인터랙션|사용자 경험|편의)\b/.test(text)) {
    return "UX";
  }
  if (/\b(business|market|plan|pm|requirement|scope|roadmap|기획|비즈니스|요구사항|시장|사업)\b/.test(text)) {
    return "Business";
  }
  if (normalizeNodePhase(phase, category) === "Solution") return "Tech";
  if (["Decision", "Action"].includes(normalizeNodePhase(phase, category))) return "Business";
  return "UX";
}

export function normalizeJobTag(value, fallback = {}) {
  return normalizeSuggestionEnum(
    value,
    NODE_JOB_TAGS,
    inferJobTag(fallback)
  );
}

export function getJobTagMeta(value) {
  const normalized = normalizeJobTag(value);
  return JOB_TAG_META[normalized] || JOB_TAG_META.UX;
}

function inferSttTopicTag(sourceType) {
  if (normalizeSourceType(sourceType) === "context") return "Context";
  if (normalizeSourceType(sourceType) === "user") return "STT";
  return "Memory";
}

function inferGeneralTopicTag(sourceType) {
  if (normalizeSourceType(sourceType) === "user") return "Input";
  if (normalizeSourceType(sourceType) === "agent") return "AI";
  return "Collaboration";
}

export function normalizeTopicTag(value, { sourceType, meetingMode = "general" } = {}) {
  if (typeof value === "string" && value.trim()) {
    return value.trim();
  }
  return meetingMode === "stt" ? inferSttTopicTag(sourceType) : inferGeneralTopicTag(sourceType);
}

export function getTopicTagMeta(value) {
  return TOPIC_TAG_META[value] || { bg: "#D4CDF1" };
}

export function isSttMeetingContext({ projectId, nodeId, data } = {}) {
  return (
    data?.meetingMode === "stt" ||
    projectId === STT_MEETING_PROJECT_ID ||
    (typeof nodeId === "string" && nodeId.startsWith("node-meet-")) ||
    Boolean(data?.originalContent)
  );
}

export function resolveTopicTagForNode({ projectId, nodeId, data } = {}) {
  const meetingMode = isSttMeetingContext({ projectId, nodeId, data }) ? "stt" : "general";
  return normalizeTopicTag(data?.topicTag, {
    sourceType: data?.sourceType,
    meetingMode,
  });
}

function looksLikeSpeechText(text) {
  if (typeof text !== "string" || !text.trim()) return false;
  return text.length > 90 || /[?？]|잖아요|더라고요|할까요|거든요|어요\?|죠\?/.test(text);
}

export function splitOriginalSpeechParagraphs(text) {
  if (typeof text !== "string" || !text.trim()) return [];
  const trimmed = text.trim();
  const breakAfterShare = trimmed.indexOf("공유하게 되는데,");
  if (breakAfterShare !== -1) {
    const first = trimmed.slice(0, breakAfterShare + "공유하게 되는데,".length).trim();
    const remainder = trimmed.slice(breakAfterShare + "공유하게 되는데,".length).trim();
    const breakAfterHard = remainder.indexOf("어렵더라고요.");
    if (breakAfterHard !== -1) {
      const second = remainder.slice(0, breakAfterHard + "어렵더라고요.".length).trim();
      const third = remainder.slice(breakAfterHard + "어렵더라고요.".length).trim();
...
export const ORIGINAL_SPEECH_HIGHLIGHT_PHRASE = "생각 맥락을 공유하는";

function summarizeNodeContent(content, title) {
  if (typeof content !== "string" || !content.trim()) return title || "";
  const trimmed = content.trim();
  const firstSentence = trimmed.split(/(?<=[.!?？])\s+/)[0]?.trim() || trimmed;
  if (firstSentence.length >= 20 && firstSentence.length < trimmed.length) {
    return firstSentence;
  }
  if (title && title.trim() && title.trim() !== trimmed) {
    return title.trim();
  }
  return trimmed.length > 90 ? `${trimmed.slice(0, 87)}...` : trimmed;
}

function ensureNodeSummaryFields(data = {}) {
  const title =
    (typeof data.title === "string" && data.title.trim()) ||
    (typeof data.label === "string" && data.label.trim()) ||
    "";
  let content = typeof data.content === "string" ? data.content.trim() : "";
  let originalContent = typeof data.originalContent === "string" ? data.originalContent.trim() : "";
  let originalTitle = typeof data.originalTitle === "string" ? data.originalTitle.trim() : "";

  if (originalContent && content === originalContent) {
    content = summarizeNodeContent(content, title);
  } else if (!originalContent && looksLikeSpeechText(content)) {
    originalContent = content;
    originalTitle = originalTitle || (title ? `${title} (원문)` : "원문");
    content = summarizeNodeContent(content, title);
  }
...
export function normalizeNodeData(data = {}, options = {}) {
  const category = normalizeNodeCategory(data.category);
  const visibility = normalizeVisibility(data.visibility);
  const legacyJobTag = NODE_JOB_TAGS.includes(data.phase) ? data.phase : data.jobTag;
  const phase = normalizeNodePhase(
    NODE_JOB_TAGS.includes(data.phase) ? undefined : data.phase,
    category
  );
  const summaryFields = ensureNodeSummaryFields(data);
  const meetingMode = isSttMeetingContext({
    projectId: options.projectId,
    nodeId: options.nodeId,
    data: { ...data, ...summaryFields, meetingMode: options.meetingMode || data.meetingMode },
  })
    ? "stt"
    : "general";
  ...
  return {
    ...data,
    title: summaryFields.title || data.title,
    content: summaryFields.content,
    originalContent: summaryFields.originalContent,
    originalTitle: summaryFields.originalTitle,
    category,
    phase,
    ...
    topicTag: normalizeTopicTag(data.topicTag, {
      sourceType: data.sourceType,
      meetingMode,
    }),
    meetingMode,
    isOffMeeting: data.isOffMeeting !== undefined ? Boolean(data.isOffMeeting) : false,
