...
export const NODE_PORT_LAYOUT = {
  toggleHeight: 22.76,
  headerGap: 8,
  cardHeight: 165,
  shellHeight: NODE_CARD_TOKENS.minHeight,
  sidePortY: 53,
  portRingOffset: 7.23,
  continuationGapY: 100,
};

export function getThinkingNodeWidth(node) {
  return node?.style?.width ?? NODE_CARD_TOKENS.width;
}

export function getThinkingNodeHeight(node) {
  return node?.style?.height ?? node?.style?.minHeight ?? NODE_PORT_LAYOUT.shellHeight;
}
...
export function toReactFlowNode(n, highlightedId, options = {}) {
  const normalizedData = normalizeNodeData(n.data, { ...options, nodeId: n.id });
  const nodeData = {
    title: n.data.label || n.data.title || normalizedData.title || "",
    content: normalizedData.content,
    originalTitle: normalizedData.originalTitle || "",
    originalContent: normalizedData.originalContent || "",
    phase: normalizedData.phase,
    category: normalizedData.category,
    jobTag: normalizedData.jobTag,
    topicTag: normalizedData.topicTag,
    meetingMode: normalizedData.meetingMode,
    isOffMeeting: normalizedData.isOffMeeting,
