...
function serializeThinkingNode(node) {
  return {
    ...
    data: sanitizeJsonValue({
      ...
      originalTitle: node?.data?.originalTitle || "",
      originalContent: node?.data?.originalContent || "",
      category: node?.data?.category,
      phase: node?.data?.phase,
      jobTag: node?.data?.jobTag,
      topicTag: node?.data?.topicTag,
      meetingMode: node?.data?.meetingMode,
      isOffMeeting: Boolean(node?.data?.isOffMeeting),
...
export function serializeProjectGraph(nodes = [], edges = [], stage = "Idea") {
  return {
...
function hydrateThinkingNode(node, options = {}) {
...
export function hydrateProjectNodes(nodes = [], options = {}) {
  return (Array.isArray(nodes) ? nodes : []).map((node) => {
    if (node?.type === "thinkingNode") return hydrateThinkingNode(node, options);
