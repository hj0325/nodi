...
export function shiftClusterRightOfExisting(existingNodes, incomingNodes) {
  ...
  const desiredMinX = existingBounds.maxX + 140;
...
export function shiftClusterBelowAnchor(anchorNode, incomingNodes, allNodes = [], existingEdges = []) {
  ...
  const incomingBounds = computeNodeBounds(incomingNodes);
  if (!anchorBounds || !incomingBounds || !anchorNode) return incomingNodes;

  const anchorId = anchorNode.id;
  const childNodes = (Array.isArray(allNodes) ? allNodes : []).filter((node) =>
    (Array.isArray(existingEdges) ? existingEdges : []).some(
      (edge) => edge?.source === anchorId && edge?.target === node?.id && getEdgeContinuationFlag(edge)
    )
  );
  const stackBounds = computeNodeBounds([anchorNode, ...childNodes]);
  ...
  return incomingNodes.map((node, index) => ({
    ...
    position: {
      x:
        ...
        (index - (incomingNodes.length - 1) / 2) * fanStep,
