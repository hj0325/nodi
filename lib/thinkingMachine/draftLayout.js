...
export const GROUP_THINKING_BASE_Y = 28;
...
export function layoutThinkingNodesInGroup(rfNodes, groupW) {
  const CATEGORY_ORDER = [
    ...
    "Problem", "Goal", "Insight", "Evidence", "Assumption", "Constraint", "Idea", "Option", "Risk", "Conflict", "Decision", "OpenQuestion"
  ];
  const rowMap = new Map(CATEGORY_ORDER.map((c, i) => [c, i]));
  const colX = { Problem: 24, Solution: 320 };
  const baseY = GROUP_THINKING_BASE_Y;
  const rowGap = 178;
  const slotGap = 54;
  ...
  (Array.isArray(rfNodes) ? rfNodes : []).forEach((n) => {
    ...
    const row = rowMap.get(cat) ?? CATEGORY_ORDER.length;
  ...
  // Collect all active row numbers and compact them to eliminate empty vertical gaps
  const activeRows = new Set();
  byRowCol.forEach((arr, key) => {
    const [rowStr] = key.split(":");
    activeRows.add(Number(rowStr));
  });

  const sortedActiveRows = [...activeRows].sort((a, b) => a - b);
  const rowToCompactIndex = new Map(sortedActiveRows.map((originalRow, seqIndex) => [originalRow, seqIndex]));

  const out = [];
  byRowCol.forEach((arr, key) => {
    const [rowStr, col] = key.split(":");
    const originalRow = Number(rowStr);
    const compactRow = rowToCompactIndex.get(originalRow) ?? 0;
    arr.forEach((n, idx) => {
      const x = Math.min(colX[col] + idx * slotGap, safeRight);
      const y = baseY + compactRow * rowGap;
