...
export function toConnectorEdges(rawEdges, nodeList, currentEdges = []) {
  ...
  return normalizedEdges.map((normalizedEdge) => {
    ...
    const sourceY = sourceNode?.position?.y ?? 0;
    const targetY = targetNode?.position?.y ?? 0;
    const sourceIsOffMeeting = Boolean(offMeetingMap.get(normalizedEdge.source));
    const targetIsOffMeeting = Boolean(offMeetingMap.get(normalizedEdge.target));
    const useVerticalHandles =
      getEdgeContinuationFlag(normalizedEdge) && targetY > sourceY + 40;
    const sourceHandle = (useVerticalHandles && sourceIsOffMeeting) ? BOTTOM_SOURCE_HANDLE_ID : SOURCE_HANDLE_ID;
    const targetHandle = (useVerticalHandles && targetIsOffMeeting) ? TOP_TARGET_HANDLE_ID : TARGET_HANDLE_ID;
    ...
    return {
      ...
      data: {
        ...
        sourceIsOffMeeting: Boolean(offMeetingMap.get(normalizedEdge.source)),
        targetIsOffMeeting: Boolean(offMeetingMap.get(normalizedEdge.target)),
      },
    };
  });
}
...
export function decorateConnectorEdges(edges = [], alignmentAnalysis = null, nodeList = []) {
  ...
  return (Array.isArray(edges) ? edges : []).map((edge) => {
    ...
    return {
      ...
      data: {
        ...(edge?.data || {}),
        sourceIsOffMeeting: Boolean(offMeetingMap.get(edge?.source)),
        targetIsOffMeeting: Boolean(offMeetingMap.get(edge?.target)),
        ...
        alignmentLineDash:
          edge?.data?.isContinuation ||
          offMeetingMap.get(edge?.source) ||
          offMeetingMap.get(edge?.target)
            ? "5 5"
