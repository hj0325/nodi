...
const BOTTOM_SOURCE_HANDLE_ID = "bottom-source";
const TOP_TARGET_HANDLE_ID = "top-target";
...
export function toConnectorEdges(rawEdges, nodeList, currentEdges = []) {
  ...
  return normalizedEdges.map((normalizedEdge) => {
    ...
    const sourceHandle = useVerticalHandles ? BOTTOM_SOURCE_HANDLE_ID : SOURCE_HANDLE_ID;
    const targetHandle = useVerticalHandles ? TOP_TARGET_HANDLE_ID : TARGET_HANDLE_ID;
  ...
  return (Array.isArray(edges) ? edges : []).map((edge) => {
    ...
    return {
      ...
      data: {
        ...
        alignmentLineDash: edge?.data?.isContinuation ? "5 5" : (visualMeta.lineDash || undefined),
      },
    };
  });
}
