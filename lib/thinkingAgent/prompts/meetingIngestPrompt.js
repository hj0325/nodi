export function buildMeetingIngestPrompt({
  ...
  const placementRule = isOffMeeting
    ? `현재 회의 상태: 종료(off-meeting). 새 노드는 기존 회의록 노드 중 발화와 가장 맞는 existing_node_id 아래에 연결되는 continuation 노드로 생성하세요. relation_label은 refines/depends_on/supports 중 맥락에 맞게 선택하고, existing_node_id를 반드시 지정하세요.`
    : `현재 회의 상태: 진행 중(in-meeting). 새 노드는 회의 흐름상 오른쪽으로 이어지는 노드로 생성하세요. existing_node_id는 직전/관련 회의 발화 노드를 연결할 때만 사용하고, 회의 중에는 continuation 연결이 아닙니다.`;

  const systemPrompt = `
당신은 STT 기반 실시간 회의 내용을 분석하여 5W1H (누가, 언제, 어디서, 무엇을, 왜, 어떻게) 및 회의 단계별 노드 중심의 회의록을 구성하는 실시간 회의 분석 엔진입니다.
...
- 회의 상태: ${isOffMeeting ? "종료 (off-meeting)" : "진행 중 (in-meeting)"}
...
5. 각 단위에 대해 해당 내용이 어느 직군과 관련되었는지 판단하여 jobTag에 다음 중 정확히 하나를 할당합니다: "Business", "UX", "Tech".
6. 각 단위에 대해 실행할 작업을 선택합니다:
...
7. 발화 내용이 기존 노드와 거의 중복되는 경우, 중복 노드를 생성하는 대신 해당 existing_node_id에 대해 strengthen 작업을 선호하세요.
8. 기존 노드가 명확한 앵커 역할을 할 때만 existing_node_id를 사용하세요.
9. relation_label은 반드시 다음 중 하나여야 합니다: supports, contradicts, causes, refines, depends_on, proposes, blocks.
10. 노드 본문(content)은 STT 원문을 그대로 넣지 말고, 1~2문장으로 요약한 노드화된 내용만 작성하세요.
11. STT 원문은 originalContent에, 원문 기반 제목은 originalTitle에 저장하세요. label/content와 originalContent는 절대 동일하게 작성하지 마세요.
12. 중요: 모든 사용자에게 노출되는 텍스트 필드(label, content, current_direction)는 반드시 한국어(Korean)로만 작성해야 합니다. 영어로 작성하지 마세요. 레이블(label)은 명확하고 간결하며 행동 지향적인 한국어로 작성하세요.
13. off-meeting(회의 종료)일 때는 What/Why/How/Problem 등 기존 회의 맥락과 가장 관련 높은 existing_node_id를 고르고, in-meeting(회의 진행 중)일 때는 최근 회의 발화 흐름에 맞는 existing_node_id를 고르세요.
...
노드 태그 기준 (STT 기반 AI 회의록 전용):
- 첫 번째 태그 (visibility): Discussion(논의중)=candidate/private, Accepted(통과)=shared/reviewed/agreed, Rejected(미통과)=rejected
- 두 번째 태그 (topicTag): Context / Memory / STT 중 정확히 하나. Context=배경·맥락, Memory=합의·기억·재참조, STT=발화자 직접 발언
- 세 번째 태그 (jobTag): Business, UX, Tech
  ...
  "units": [
    {
      ...
      "content": "노드 요약 본문 (한국어, 1~2문장, STT 원문 아님)",
      "originalTitle": "STT 원문 기반 제목 (한국어, 선택)",
      "originalContent": "STT 원문 전체 (한국어)",
      "category": "Who|When|Where|What|Why|How",
      "phase": "Idea|Research|Solution|Decision|Action",
      "jobTag": "Business|UX|Tech",
      "topicTag": "Context|Memory|STT",
      "ownerId": "string",
      "sourceType": "user|agent|mixed|context",
