...
export function toNode({
  ...
  return {
    ...
    data: {
      ...
      jobTag: normalizeJobTag(jobTag, {
        category: normalizedCategory,
        phase: normalizedPhase,
        title: label,
        content,
      }),
      topicTag: normalizeTopicTag(topicTag, {
        sourceType,
        meetingMode: originalContent ? "stt" : "general",
      }),
      ...
      isOffMeeting: Boolean(isOffMeeting),
    },
    position,
  };
}

export function toEdge({ id, source, target, label = "refines", isContinuation = false }) {
  return { id, source, target, label: normalizeRelationLabel(label), isContinuation };
}
