  ...
  return rawNodes
    .map((node) => {
      ...
      const phase = normalizePhase(
        NODE_JOB_TAGS.includes(node?.phase) ? undefined : node?.phase,
        category
      );
      const legacyJobTag = NODE_JOB_TAGS.includes(node?.phase) ? node.phase : node?.jobTag;
      return {
        ...
        topicTag: normalizeTopicTag(node?.topicTag, {
          sourceType: node?.sourceType,
          meetingMode: node?.originalContent ? "stt" : "general",
        }),
        originalTitle: typeof node?.originalTitle === "string" ? node.originalTitle.trim() : "",
        originalContent: typeof node?.originalContent === "string" ? node.originalContent.trim() : "",
